...
PrepareTransitionList <- function(mainPath,maxquant,inclusionList,ppm = 10,ppm2 = 10,threads = 2,session= NULL,pythonpath = NULL,test = F,useDIA=F){
  ...
  # path <- "E:/Projects/20191220_MVB_SILAC_PRM"
  # inclusionList <- "E:/Projects/20191220_MVB_SILAC_PRM/Picky_Export_PRM_Thermo_Q_Exactive-2/"
  ...
  # mainPath <- "/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/20200317_MVB_HCT116BenchmarkMix/20200529/HCT/"
  maxquant <- NULL
  # session <- NULL
  wd <- getwd()
  # mainPath <- "/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/ERIK/20200210_Mito/Picky/NewTest_PRM_TargetDIA/tDIA10"
  # mainPath <- "/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/ERIK/20200210_Mito/Picky/20200208_ETC_Picky_Test/Exploris_FAIMS"
...
## RawDiage Extractor Function
# setwd("/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/20200509_targetDIA/Raw/")
# r <- "Beaker_20200507_HZ_HS_TargetDIA_Test_1.raw"
# RawDiag_Vali_Extracter <- function(r,dirout,outpath,ST,TT,TTdec,ppm1=10,ppm2=10,ShowInfo=F,extraSink=T,SystemPath=NULL,session=NULL){
#   # if(extraSink){
#   #   sink("SessionSink.txt",split = T,type="message")
#   # }
#   #
#   # setwd("/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/20200509_targetDIA/ThirdTrial_QE")
#   # r <- "QE_run/Animal_20200617_HZ_HS_tDIA_test_1.raw"
#   # dirout <- "./"
#   # ST <- fread("Picky_Export_FocusDIA_Thermo_Q_Exactive/SpectraTable.txt",sep = "\t")
#   # TT <- fread("Picky_Export_FocusDIA_Thermo_Q_Exactive/TransitionTable.txt",sep = "\t")
#   require(rawDiag)
#   require(compiler)
#   require(data.table)
#   require(Peptides)
...
#     
#     
#     # this loop goes throguh the raw files
#     setwd(outpath)
#     r <<- r
...
#         })  
#         Workeddt <- rbindlist(Worked) # data.table with fragments in the list
#         if(ShowInfo){
#           
...
#           if(wi[1]<= mean(lib_precursor)&wi[2]>= mean(lib_precursor)){
#             # Extract Masses
#             if(ShowInfo){
#               print(gr$SpecID)
#               
...
# }
# 
# RawDiag_Vali_Extracter_DIA_v1 <- function(r,dirout,outpath,ST,TT,TTdec,ppm1=10,ppm2=10,ShowInfo=F,extraSink=T,SystemPath=NULL,session=NULL,DIA=T){
#   # if(extraSink){
#   #   sink("SessionSink.txt",split = T,type="message")
#   # }
#   #
...
#   
# }
# RawDiag_Vali_Extracter_DIA_v2 <- function(r,dirout,outpath,ST,TT,TTdec,ppm1=10,ppm2=10,ShowInfo=F,extraSink=T,SystemPath=NULL,session=NULL,DIA=T){
#   # if(extraSink){
#   #   sink("SessionSink.txt",split = T,type="message")
#   # }
#   #
...
# dirout <- "./"
# InclusionlistPath <- "/Users/henno/Documents/Skripte/R-Functions/Selbach-Functions/DataAnalysis/ERIK/20200210_Mito/Picky/NewTest_PRM_TargetDIA/tDIA5/Picky_Export_FocusDIA_Thermo_Exploris_5Da"
# ST <- fread("Picky_Export_FocusDIA_Thermo_Q_Exactive/SpectraTable.txt",sep = "\t")
# TT <- fread("Picky_Export_FocusDIA_Thermo_Q_Exactive/TransitionTable.txt",sep = "\t")
  ...
  FUN <-try({
    ...
    sapply(1:length(fi),function(itx){
      ...
      if(length(unique(mztable$Score))<2|ReScore){
        ...
        if(Reverse){
          fwrite(mztableSelect,file = "REV_Temp_CheckFile.txt",sep = "\t",append = T)
        }else{
          fwrite(mztableSelect,file = "Temp_CheckFile.txt",sep = "\t",append = T)
    ...
    for(txt in c("Temp_CheckFile.txt","REV_Temp_CheckFile.txt")){
      ...
      mztableSelect[,{
        ...
        temp$GBM_Scores <- RF_Scores
        temp$DL_Scores <- DL_Scores
        temp <- temp
  ...
  FUN <-try({
    ...
    for(txt in c("Temp_CheckFile.txt","REV_Temp_CheckFile.txt")){
      ...
      mztableSelect[,{
        ...
        da <- .SD
        temp <- dbReadTable(db,gr$mzname)
        MatchFun <- match(temp$scan,scan)
        temp$RF_Scores <- RF_Scores[MatchFun]
        temp$DL_Scores <- DL_Scores[MatchFun]
...
## PEAKS Finding MODUL
RTcandidatesFun <- function(ana,FDRcutoff,RTranges = NULL,supersmooth=F,supersmooth_bw=0.2){
  if(length(RTranges)!=0){
    ...
    RTcandidates <- lapply(ana,function(x){
      ...
      x[x$FDR<=FDRcutoff,.(rawfile,RT_Used,DL_Scores),]
  ...
  for(i in 1:dim(x)[2]){
    ...
    if(plot.transition){
      ...
      if(length(a)>maximalnumber){
        # print("Decreasing number of datapoints")
...
plot.analyzed.transition <- function(x,type = "Volcano",yl = NULL,xl = NULL,Ppos = NULL,add = F,FDRCutOff = 0.01,TransitionColors = T,colAll = 2,secPlotType = "Intensity Correlation",p.value = 0.01,RetTime = NULL,DDA = 1:3,PRMonly = T,AddSecondPlot = T,onlySignificant = F,centerPeak = F,colmap=NULL,blankplot = F,FDRType = "FDR",
  ...
  if(type == "Volcano"){
    ...
    print("Finished plotTransitions")
...
PeakDetection <- function(transitions,RTs,movavg_window=5,movavg_type="t",QualityPosition=NA,summary_bw=0.1,ZeroSmooth=T,Diff_Smooth=T,supersmooth_I=F,supersmooth_bw=0.2){
  ...
  # points(RTs,transitions$y12/max(transitions$y3),type="l")
  # 
  d_f <- findpeaks(d$y,nups = 1,ndowns = 1)[,2]
  PeakCandidates <- d$x[d_f]
  tfu[,ExpectedPeak:={
    ca <- .BY$V2
    cadiff <- abs(ca-PeakCandidates)
    PeakCandidates[which(cadiff==min(cadiff))]
  },V2]
  PeakSummary <- tfu[,{
    peak=median(V2,na.rm = T)
    peaksd <- sd(V2,na.rm = T)
    end=median(end,na.rm = T)
    start=median(start,na.rm = T)
    list(peak=peak,peaksd=peaksd,start=start,end=end,n=length(V2),MaxIntensity=    max(s,na.rm = T))
  },ExpectedPeak]
...
# Peaks <<- PeakDetection(transitions,RT.DetectPeak,movavg_window=5,movavg_type="t",ZeroSmooth = F,Diff_Smooth = F)


# PeakDetection <- function(transitions,RTs,movavg_window=5,movavg_type="t",QualityPosition=NA,summary_bw=0.1,ZeroSmooth=T,Diff_Smooth=T){
#   transitionstemp <- transitions
#   # RTs <- RT.DetectPeak
#   if(ZeroSmooth){
#     transitionstemp[transitionstemp==0] <- NA
#     transitionstemp <- Neighbour_Mean_imputation(transitionstemp)
#   }
#   
#   if(Diff_Smooth){
...
#   }
#   
#   temp <- apply(transitionstemp,2,function(x){
#     x <<- x
#     try({
#       if(sd(diff(x[!is.na(x)],na.rm = T))>10){
#         movavg_window <- 10
#         # x[x==0] <- NA
#         
#       }
#     })
#    
#     x<- movavg(x,movavg_window,movavg_type)
#     te <- data.table(findpeaks(x,nups = 1,ndowns = 1))
#     te
#   })
#   
#   temp <- rbindlist(temp)
...
#   tfu <- temp[,{
#     data.table(s=sum(V1),
#                start=round(quantile(RTs[V3],probs=0.2)),
#                end=round(quantile(RTs[V4],probs=0.8)))
#     
#   },.(V2=RTs[V2])]
#   tfu <<- tfu
#   d <- density(tfu$V2,bw=summary_bw)
#   d_f <- findpeaks(d$y)[,2]
#   PeakCandidates <- d$x[d_f]
#   tfu[,ExpectedPeak:={
#     ca <- .BY$V2
...
DetectPeak <- function(rt_pep,Peakwidth,transitions,RT,
  ...
  if(length(presetQuantiles) != 2&length(Seli) > 0){
    ...
    # PS2 <- Peaks$Peaks
    # checking boundaries based on similar Peaks
    # PS2$RTdiff <- PS2$V2-qua[3]
    # qua <- PS2[RTdiff<MaxPeakWidth/2,.(quantile(start,0.1),quantile(end,0.9),qua[3])]
    # qua <- unlist(qua[1,])[1:3]
    # 
    ######
    # qua_qua <- PS[(rt_pep>=start&rt_pep<=end)|RTdiff<=0.5]
    # qua <- qua_qua[MaxIntensity>=quantile(PS$MaxIntensity,0.5)]
    # qua <- qua_qua[order(ExpectedPeak)]
    # if(diff(range(qua$ExpectedPeak))<1/60*10){
    #   qua <- qua[,.(quantile(start,0.1),quantile(end,0.9),median(peak,na.rm = T))]
    #   
    # }else{
    #   qua <- qua[MaxIntensity==max(MaxIntensity)]
    #   qua <- qua[,.(quantile(start,0.1),quantile(end,0.9),median(peak,na.rm = T))]
    #   
    # }
    # 
    # qua <- unlist(qua[1,])
    # if(all(is.na(qua[1:2]))){
    #   # if best spectrum match is not within boundaries, check among Peaks table
    #   qua <- Peaks$Peaks[rt_pep>=start&rt_pep<=end]
    #   qua <- qua[s==max(s)]
    #   qua <- qua[,.(quantile(start,0.1),quantile(end,0.9),median(V2,na.rm = T))]
    #   qua <- unlist(qua[1,])
    #   if(all(is.na(qua[1:2]))){
    #     # after check within 20s boundary
    #     difu1 <- abs(Peaks$Peaks$start-rt_pep)
    #     difu2 <- abs(Peaks$Peaks$end-rt_pep)
    #     
    #     cutof <- 1/60*20
    #     if(any(difu1< cutof)|any(difu2<cutof)){
    #       mifu <- apply(cbind(difu1,difu2),1,min)
    #       qua <- Peaks$Peaks[mifu==min(mifu),]
    #       qua <- qua[,.(quantile(start,0.1),quantile(end,0.9),median(V2,na.rm = T))]
    #       qua <- qua[s==max(s)]
    #       qua <- unlist(qua[1,])
    #     }
...
DetectPeakWrapper <- function(
  ...
  if(all(unlist(dblistT(db =dbp)) != dbtaNameVec)|Reanalysis){
    ...
    # print("Start Writing Peakfun Peaksfun")
    # DP_PeaksFun<- DP
...
PeaksProcessingLM <- function(peaks,Remove_y1 =T,Plot=T){
  ...
  pdf("./export/PeakView.pdf",width = 7)
  ...
  RatioCalc <-peaks[,{
    ...
    if(length(UNI) >1){
      ...
      HUM$rf <- gr$rawfile
      # print("STUFF1")
      # try( CollectFun <<- rbind(CollectFun,HUM),silent = T)
      # print(dim(CollectFun))
      ...
      if(dim(HUM)[1]==0#|all(is.na(HUM$Ratio))
        ...
        RatioOut2 <- NULL
        # stop()
        # print(paste("EMPTY for",unique(HUMbackup$Label1)))
      }else{
        RatioOut2 <-HUM[,{
